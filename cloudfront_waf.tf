resource "aws_wafv2_web_acl" "trip_planner_cloudfront_waf" {
  ...
  dynamic "rule" {
    for_each = var.blocked_bots
    content {
      name     = "Block${rule.value}"
      priority = 10 + index(var.blocked_bots, rule.value) # avoid conflicts with managed rules
  ...
  rule {
    name     = "RateLimitPerIP"
    priority = 10 + length(var.blocked_bots) + 1 # after bot rules
