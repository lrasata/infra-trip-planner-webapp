...
module "cloudfront" {
  ...
  alb_lb_dns_name                            = data.terraform_remote_state.backend_app.outputs.alb_lb_dns_name
  ...
  locations_auth_secret                      = data.terraform_remote_state.security.outputs.locations_auth_secret
  ...
  uploads_bucket_regional_domain_name        = data.terraform_remote_state.backend_app.outputs.uploads_bucket_regional_domain_name
}
...
module "uploads_bucket_policy" {
  ...
  bucket_id      = data.terraform_remote_state.backend_app.outputs.uploads_bucket_id
  bucket_arn     = data.terraform_remote_state.backend_app.outputs.uploads_bucket_arn
  cloudfront_arn = module.cloudfront.cloudfront_arn
...
module "locations_api" {
  ...
  sns_topic_alerts_arn      = data.terraform_remote_state.backend_app.outputs.sns_topic_arn
}
