...
resource "aws_cloudwatch_metric_alarm" "ecs_cpu_high" {
  alarm_name          = "${aws_ecs_service.ecs_service_trip_planner.name}-CPUHigh"
  ...
  dimensions = {
    ...
    ServiceName = aws_ecs_service.ecs_service_trip_planner.name
  }
}

# Memory Utilization Alarm
resource "aws_cloudwatch_metric_alarm" "ecs_memory_high" {
  alarm_name          = "${aws_ecs_service.ecs_service_trip_planner.name}-MemoryHigh"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 2
...
resource "aws_cloudwatch_metric_alarm" "ecs_unhealthy_tasks" {
  alarm_name          = "${aws_ecs_service.ecs_service_trip_planner.name}-UnhealthyTasks"
  comparison_operator = "GreaterThanThreshold"
